...
with open(os.path.join(this_dir, INFO_FILENAME), 'w') as file:

    for img_name in img_list:
        img_url = os.path.join(postive_url, img_name)
        img = cv.imread(img_url)
        des = cv.resize(img, (size, size))
        cv.imwrite(img_url, des)
        cols, rows = size,size 
...
with open(os.path.join(this_dir, BG_FILENAME), 'w') as file:
